...
?>
    ...
    <div id="hotel-booking-payment">
        ...
        <form name="hb-payment-form" id="hb-payment-form" method="post" action="<?php echo isset( $search_page ) ? $search_page : ''; ?>">
            <h3><?php _e( 'Booking Rooms', 'resort' ); ?></h3>
            <table class="hb_table">
                <thead>
                <th class="hb_room_type"><?php _e( 'Room type', 'resort' ); ?></th>
                <th class="hb_capacity"><?php _e( 'Capacity', 'resort' ); ?></th>
                <th class="hb_quantity"><?php _e( 'Quantity', 'resort' ); ?></th>
                <th class="hb_check_in"><?php _e( 'Check - in', 'resort' ); ?></th>
                <th class="hb_check_out"><?php _e( 'Check - out', 'resort' ); ?></th>
                <th class="hb_night"><?php _e( 'Night', 'resort' ); ?></th>
                <th class="hb_gross_total"><?php _e( 'Gross Total', 'resort' ); ?></th>
                </thead>
				<?php if ( $rooms = $cart->get_rooms() ): ?>
					<?php foreach ( $rooms as $cart_id => $room ): ?>
                        ...
                        <tr class="hb_checkout_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
                            ...
                            <td class="hb_capacity"><?php echo sprintf( _n( '%d adult', '%d adults', $room->capacity, 'resort' ), $room->capacity ); ?> </td>
                ...
                <tr class="hb_sub_total">
                    <td colspan="8"><?php _e( 'Sub Total', 'resort' ); ?>
				...
				<?php if ( $tax = hb_get_tax_settings() ) { ?>
                    <tr class="hb_advance_tax">
                        <td colspan="8">
							<?php _e( 'Tax', 'resort' ); ?>
							<?php if ( $tax < 0 ) { ?>
                                <span><?php printf( __( '(price including tax)', 'resort' ) ); ?></span>
							<?php } ?>
				...
				<?php if ( $advance_payment = $cart->advance_payment ) { ?>
                    <tr class="hb_advance_payment">
                        <td colspan="8">
							<?php printf( __( 'Advance Payment (%s%% of Grand Total)', 'resort' ), hb_get_advance_payment() ); ?>
                        ...
                        <tr class="hb_payment_all">
                            <td colspan="8" class="hb-align-right">
                                <label class="hb-align-right">
									...
									<?php _e( 'I want to pay all', 'resort' ); ?>
                                </label>
                            </td>
                        </tr>
					<?php } ?>
			...
			<?php if ( !is_user_logged_in() && !hb_settings()->get( 'guest_checkout' ) && get_option( 'users_can_register' ) ) : ?>
				...
				<?php printf( __( 'You have to <strong><a href="%s">login</a></strong> or <strong><a href="%s">register</a></strong> to checkout.', 'resort' ), wp_login_url( hb_get_checkout_url() ), wp_registration_url() ) ?>
				...
				<?php if ( $tos_page_id = hb_get_page_id( 'terms' ) ) { ?>
                    <p>
                        <label>
							...
							<?php printf( __( 'I agree with ', 'resort' ) . '<a href="%s" target="_blank">%s</a>', get_permalink( $tos_page_id ), get_the_title( $tos_page_id ) ); ?>
                        </label>
                    </p>
				<?php } ?>
                <p>
                    <button type="submit" class="hb_button"><?php _e( 'Check Out', 'resort' ); ?></button>
                </p>
