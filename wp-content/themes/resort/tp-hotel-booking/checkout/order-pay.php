...
<?php if ( $book->get_status() !== 'completed' ): ?>
    ...
    <h3><?php printf( __( 'Booking ID: %s', 'resort' ), hb_format_order_number( $booking_id ) ) ?></h3>
    <p>
        <strong><?php _e( 'Payment status: ', 'resort' ) ?></strong>
		<?php printf( '%s', ucfirst( $book->get_status() ) ) ?>
    </p>
    <p>
        <strong><?php _e( 'Booking Date: ', 'resort' ) ?></strong>
		<?php printf( '%s', get_the_date( '', $book->id ) ) ?>
    </p>
    <p>
        <strong><?php _e( 'Payment Method: ', 'resort' ) ?></strong>
		<?php printf( '%s', $book->method_title ) ?>
    </p>
    <p>
        <strong><?php _e( 'Total: ', 'resort' ) ?></strong>
		<?php printf( '%s', hb_format_price( hb_booking_total( $booking_id ), $currency_symbol ) ) ?>
    </p>
    <p>
        <strong><?php _e( 'Advance Payment: ', 'resort' ) ?></strong>
...
<?php else: ?>
    ...
    <h3><?php printf( __( '%s was pay completed', 'resort' ), $book->get_booking_number() ) ?></h3>
