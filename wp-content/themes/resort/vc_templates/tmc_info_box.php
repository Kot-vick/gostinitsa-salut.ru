...
if( !empty( $vc_image_size ) ) {
	...
	$image_size = 'resort-image-350x204-croped';
}

if( $style == 'style_3' ){
	$image_size = 'resort-image-350x250-croped';
}
...
<div class="info_box<?php echo esc_attr( $css_class ); ?>">
	...
	<?php if( $style == 'style_6' ) : ?>
		...
		<div class="info_box_text">
			...
			if ( $link['url'] ) {
				if ( ! $link['title'] ) {
					$link['title'] = esc_html__( 'Read more', 'resort' );
				}
				if ( ! $link['target'] ) {
					$link['target'] = '_self';
				}
				if( $icon ){
					$link['title'] = '<span>' . esc_html( $link['title'] ) . '</span>' . '<i class=" ' . esc_attr( $icon ) . ' tmc_icon"></i>';
				}
