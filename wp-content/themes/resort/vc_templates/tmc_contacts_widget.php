...
<div class="tmc_contacts_widget<?php echo esc_attr( $css_class ); ?>">
	...
	<?php elseif( $style == 'style_3' ) : ?>
		...
		<ul>
			<?php if( $phones ): ?>
				<li>
					<div class="icon"><i class="tmc-icon tmc-phone-11"></i></div>
					<?php
					...
					<?php if( !empty( $phones_arr ) && $phones_count > 0 ) : ?>
						<div class="text">
							<?php foreach( $phones_arr as $phone_number ) : $phones_showed++; ?>
								<a href="tel:<?php echo esc_attr( str_replace(' ', '', $phone_number) ); ?>"><?php echo esc_html( $phone_number ); ?></a><?php echo ( ( $phones_count > $phones_showed ) ? '<br>' : ''); ?>
							<?php endforeach; ?>
						</div>
					<?php endif; ?>
				</li>
			...
			<?php if( $email ): ?>
				<li>
					<div class="icon"><i class="tmc-icon tmc-envelope-11"></i></div>
					<div class="text"><a href="mailto:<?php echo esc_attr( $email ); ?>"><?php echo esc_html( $email ); ?></a></div>
				</li>
			<?php endif; ?>
			<?php if( $address ): ?>
				<li>
					<div class="icon"><i class="tmc-icon tmc-pin-11"></i></div>
					<div class="text"><?php echo wp_kses_post( $address ); ?></div>
				</li>
			<?php endif; ?>
			<?php if( $schedule ): ?>
				<li>
					<div class="icon"><i class="tmc-icon tmc-clock6"></i></div>
